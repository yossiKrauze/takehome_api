...
def _look_up_patient(patient_first_name: str, patient_last_name: str) -> Patient:
...
def _get_available_providers_per_state_and_insurance(state: str, insurance: str) -> list[Provider]:
    return Provider.query.where(
        Provider.states.any(State.state == state),
        Provider.insurances.any(Insurance.name == insurance)).all()


def _get_available_appointments(providers: list[Provider], patient: Patient) -> AppointmentResponse:
...
def _set_appointment(appointment_id: str, patient_id: str) -> bool:
    try:
        appointment_uuid = uuid.UUID(appointment_id)
        patient_uuid = uuid.UUID(str(patient_id))

        logger.debug(f"Looking for appointment with ID: {appointment_uuid}")
...
def _create_patient(session: Session, patient_data: dict) -> Patient:
    patient = Patient(
...
def schedule_appointment(session: Session, patient_data: dict) -> tuple[AppointmentResponse, bool]:
    try:
        # with session:
        # look up the patient
        patient = _look_up_patient(
            patient_data["first_name"], patient_data["last_name"])

        if not patient:
            patient = _create_patient(session, patient_data)
            logger.info("Created patient: %s", patient)

        logger.info("Found patient: %s", patient)

        providers = _get_available_providers_per_state_and_insurance(
            patient.state, patient.insurance)
        logger.info(
            "Found providers for the state and insurance: %s", providers)

        appointment_response = _get_available_appointments(
            providers, patient)
